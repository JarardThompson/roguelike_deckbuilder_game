...
func _generate_initial_grid() -> Array[Array]:
	...
	for i in FLOORS:
		...
		for j in MAP_WIDTH:
			...
			# Boss room as a non-random Y pos
			if i == FLOORS - 1:
...
func _would_cross_existing_path(i: int, j: int, room: Room) -> bool:
	...
	# if j == MAP_WIDTH, there's no right neighbour
	if j < MAP_WIDTH - 1:
		right_neighbour = map_data[i][j + 1]
	
	# paths can't cross in right direction if right neighbour goes left
	if right_neighbour and room.column > j:
		for next_room: Room in right_neighbour.next_rooms:
			if next_room.column < room.column:
				return true
	
	# paths can't cross in left direction if left neighbour goes right
